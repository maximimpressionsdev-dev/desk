...
import {
  hasSidebarOpenCookie,
  persistSidebarOpen,
  readSidebarOpenFromLocalStorage,
...
function SidebarProvider({
  ...
  React.useEffect(() => {
    if (hasSidebarOpenCookie()) return
    const stored = readSidebarOpenFromLocalStorage(defaultOpen)
    if (stored === defaultOpen) return
    if (setOpenProp) {
      setOpenProp(stored)
    } else {
      _setOpen(stored)
    }
    // One-time client restore when cookies are missing (e.g. cleared) but localStorage remains.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- mount-only hydration from storage
  }, [])
